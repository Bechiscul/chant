...
void assert_tokens(Token *a, char *b) {
...
int main() {
  // Symbols and Operators
  {
    ...
    assert_tokens(expect, "foo + bar");
  }

  // Strings
  {
    Token expect[1] = {{TK_STRING, "bruh \\\"\\\" ! "}};
    assert_tokens(expect, "\"bruh \\\"\\\" ! \"");
  }

  {
    Token expect[3] = {
        {TK_STRING, "bruh dah"}, {TK_PLUS, "+"}, {TK_STRING, "foo bar"}};
    const char *test_string = "\"bruh dah\" + \"foo bar\"";
  }
}
